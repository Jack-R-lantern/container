/* -*- compile-command: "gcc -Wall -Werror contained.c -o contained -lcap -lseccomp" -*- */
...
finish_options:
	...
	if (major != 5 || (minor != 10 && minor != 11)) {
		fprintf(stderr, "expected 5.10.x or 5.11.x: %s\n", host.release);
		goto cleanup;
	}
	if (strcmp("aarch64", host.machine)) {
		fprintf(stderr, "expected aarch64: %s\n", host.machine);
		goto cleanup;
	}
